...
for iter in range(Niter):
    ...
    plt.title(f"Estimate (iter {iter})")
    plt.axis("equal")

    plt.subplot(2, 2, 3)
    plt.imshow(search_dir, extent=[xi.min(), xi.max(), yi.max(), yi.min()], cmap="gray")
    plt.title("Search Dir")
    plt.axis("equal")

    plt.subplot(2, 2, 4)
    plt.imshow(
        -gradient_img, extent=[xi.min(), xi.max(), yi.max(), yi.min()], cmap="gray"
    )
    plt.title("Gradient")
    plt.axis("equal")

...
# NOT POSSIBLE to implement custom_jvp and custom_vjp for the same function:
# https://github.com/jax-ml/jax/issues/3996
# Ignores the second decorator (jax.custom_jvp) below
@jax.custom_vjp
@jax.custom_jvp
...
def forward_jvp(primal_in, tangent_in):
    ...
    dusim_full = simulate(t, pertSrc, masses_in, springs_in);
...
# Hessian-Vector Product (HVP) works - via jax.grad and jax.vjp because the
# HVP was implemented forward-over-reverse [jax.jvp applied to jax.grad(loss_function)]
loss_function_new = lambda params: loss_function(params, umeas)
primal_out, tangent_out = hvp(loss_function_new, ((masses_in, springs_in),), ((dmasses_in, dsprings_in),))

# However jax.jvp cannot be applied directly to the forward model!
...
primal_out, tangent_out = jax.jvp(forward_model, ((masses_in, springs_in),), ((dmasses_in, dsprings_in),))

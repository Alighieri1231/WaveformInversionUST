...
masses = 10*ones(N,1); % Random unknown masses
...
% Initial Guess for Springs
springs_estim = 10*ones(N+1,1);

% (Nonlinear) Conjugate Gradient
flatten = @(v) v(:);
search_dir = zeros(N+1,1); % Conjugate Gradient Direction
grad_prev = zeros(N+1,1); % Previous Gradient Image
for iter = 1:Niter
    % Step 1: Calculate Gradient/Backprojection
    % (1A) Forward Simulation
    usim_full = simulate(t, force, masses, springs_estim);
    usim = usim_full(meas_idx, :);
    % (1B) Virtual Sources
    virtSrc = virtualSourceSprings(usim_full, t, springs_estim);
    % (1C) Build Adjoint Sources
    adj_force = zeros(N, Nt); adj_force(meas_idx,:) = usim - umeas;
    % (1D) Solve Adjoint Wave Equation
    adj_sim_full = adjoint_sim(t, adj_force, masses, springs_estim);
    % (1E) Calculate Gradient/Backprojection
    grad = -flatten(sum(conj(virtSrc).*adj_sim_full,[1,2]));
    ...
    pertSrc = -sum(virtSrc .* reshape(search_dir,[1,1,N+1]), 3);
    dusim_full = simulate(t, pertSrc, masses, springs_estim);
    ...
    springs_estim = springs_estim + stepSize * search_dir;
    % Visualize Reconstructed Solution
    subplot(1,3,1); plot((x(1:end-1)+x(2:end))/2, springs, 'k', ...
